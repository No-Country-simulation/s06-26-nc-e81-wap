...
import { WidgetCard as WidgetCardContainer } from "@/shared/ui/WidgetCard";
...
export function WidgetCard({ widget, isLoading }: WidgetCardProps) {
  ...
  if (isLoading) {
    return (
      <WidgetCardContainer icon={Brain}>
  ...
  if (!widget || widget.isEmpty) {
    ...
    return (
      <WidgetCardContainer icon={Icon} label={widget ? t(`nav.${widget.domain}`) : undefined}>
        {widget?.isEmpty && (
